...
(function (win, doc) {
    ...
    let defaultConfig = {
        "totalVotes": 10000,                                // 总投票数量（有效的投票数量到达该数量后会停止投票）
        "intervalSeconds": 1,                               // 投票的间隔时间（秒）
        "randomInterval": [                                 // 随机间隔（毫秒）[ 起始值, 结束值, 权重 ]
            [1000, 2000, 100],
            [400, 800, 30],
            [2000, 5000, 10],
            [5000, 10 * 1000, 2],
    ...
    let utils = {
        ...
        "getUrlParamsMap": function () {
            let a = location.search.trim().replace("?", "").split("&");
            let b = {};
            for (let e of a) {
                let c = e.trim().split("=");
                if (c.length === 2 && c[1])
        ...
        "generateIntByWeight": function (data) {
            let total = 1;
            ...
            for (let i = 0; i < data.length; ++i) {
                ...
                if (total >= threshold) {
                    return Math.floor(Math.random() * (data[i][1] - data[i][0]) + data[i][0]);
                }
            }
        },
    };
    ...
    let common = {
        "API_URL": "https://api.businessweekly.com.tw/api/Member/AddMemberAction/",
        "doctorInfo": {},
        "queryDoctorInfo": function (qid) {
            var value = {};
            jQuery.ajax({
                "url": "https://cmsapi.businessweekly.com.tw/api/Doctor/Query/",
                "type": "POST",
                "dataType": "JSON",
                "async": false,
                ...
                "data": JSON.stringify({
                    ...
                    PageRows: 1,
                    SelectAll: "n",
                    SortField: "votecountsum",
                    SortType: "desc",
                    GetTodayVote: "y",
                    operationsource: "AcNG7LEeOGpbR4pMy",
                    Division: "0002",
                }),
                "success": function (res) {
                    if (res && res["Status"] && res["QuestionnaireTopicList"]) {
                        value = res["QuestionnaireTopicList"][0];
                        return;
                    }
                    console.log(res);
                    clog.e("Query failed: " + res["Message"]);
                },
            });
            return value;
        },
    ...
    let program = {
        ...
        "start": function (config) {
            ...
            let requestParams = {
                ...
                ItemNo: undefined,                                  // 医生的 ID（格式应该是一个 UUID 码）注意这个很重要
                LogValue: undefined,                                // 良醫四大指標（問診詳細、專業最可靠、用藥可信賴、親切有耐心）
            };

            requestParams["ItemNo"] = common.doctorInfo["QuestionnaireTopicId"];
            requestParams["LogValue"] = common.doctorInfo["questionnaireTopicOptionViewModels"].map(e => e["QuestionnaireTopicOptionId"]).join(",");
            thiz.cache["startTime"] = new Date();
            thiz.cacheConfig = config;
            thiz.once(requestParams, thiz.cacheConfig["intervalSeconds"] * 1000);
        },
        "once": function (requestParams, delay) {
            let thiz = this;
            thiz.running = true;
            thiz.currentTrigger = setTimeout(function (reqData) {
                reqData["IP"] = utils.generateIP();                                 // 随机 IP
                reqData["UUID"] = uuidv4();                                         // 随机 UUID
                reqData["LogValue"] = common.randomDoctorLog();                     // 随机 四大指標（如需固定指标 > 请注释该行）
                delay = utils.generateIntByWeight(defaultConfig.randomInterval);    // 加权随机间隔（如需固定秒 > 请注释该行）
    ...
    win.ycApi = {
        ...
        "statistics": function () {
            let doctor = common.queryDoctorInfo(common.doctorInfo["RelatedContentId"]);
            let info = {
                ...
                "Real-time votes": doctor["VoteCountSum"],
        ...
        "loadDoctor": function (qid) {
            common.doctorInfo = common.queryDoctorInfo(qid);
            if (common.doctorInfo && common.doctorInfo["TopicName"]) {
                // ignore
                return
            }
            let content = "According to '" + qid + "', no doctor information was obtained";
            clog.e(content)
        },
